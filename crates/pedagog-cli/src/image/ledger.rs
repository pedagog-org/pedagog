//! Read and write the build ledger (`/pedagog/config/build.toml`) — the resolved
...
/// Default ledger location inside the image.
pub const DEFAULT_LEDGER: &str = "/pedagog/config/build.toml";

/// Load the ledger, treating a missing file as an empty (fresh) ledger.
pub fn load(path: &Path) -> Result<BuildState> {
    if !path.exists() {
